...
use app\shop\model\GoodsTagModel;
...
class GoodsTag extends AdminController
{
    ...
    public function getList($runData = false, $is_delete = 0)
    {
        ...
        foreach ($data['list'] as $key => $val) {
            $where = [];
            $where[] = ['tag_id', '=', $val['id']];
            $data['list'][$key]['goods_num'] = $GoodsModel->where($where)->count('goods_id');
        }
    ...
    public function beforeAdd($data)
    {
        ...
        if (empty($data['title'])) return $this->error('请填写标题！');
        if (empty($data['subtitle'])) return $this->error('请填写副标题！');
        return $data;
    }
    //-- 添加后调用
    /*------------------------------------------------------ */
